...
- (void)viewDidLoad {
    ...
    [self.editPhoneBTN.titleLabel setFont:[UIFont fontWithName:@"OpenSans" size:13]];
    ...
    self.inputBoxes = @[self.inputBox1, self.inputBox2, self.inputBox3, self.inputBox4];
...
- (BOOL)textField:(UITextField *)textField shouldChangeCharactersInRange:(NSRange)range replacementString:(NSString *)string {
    ...
    if ([textField isEqual:self.invisInputField]) {
        // All digits entered
        if (range.location == 3) {
            NSString *enteredCode = [NSString stringWithFormat:@"%@%@%@%@", self.inputBox1.text, self.inputBox2.text, self.inputBox3.text, self.inputBox4.text];
            NSLog(@"Entered code is %@", enteredCode);
            if ([enteredCode isEqualToString:self.verificationCode]) {
                [[SettingsHelper sharedSettingsHelper] storePhoneNumber:self.phoneTF
                 .text];
...
- (IBAction)clickedSend {
    ...
    [UIView animateKeyframesWithDuration:0.2 delay:0 options:0 animations:^{
        ...
        CGAffineTransform box1Translation = CGAffineTransformTranslate(self.inputBox1.transform, -320, 0);
        CGAffineTransform box2Translation = CGAffineTransformTranslate(self.inputBox2.transform, -320, 0);
        CGAffineTransform box3Translation = CGAffineTransformTranslate(self.inputBox3.transform, -320, 0);
        CGAffineTransform box4Translation = CGAffineTransformTranslate(self.inputBox4.transform, -320, 0);
        [UIView animateKeyframesWithDuration:0.2 delay:0.1 options:0 animations:^{
            self.inputBox1.transform = box1Translation;
        } completion:^(BOOL finished) {}];
        [UIView animateKeyframesWithDuration:0.2 delay:0.2 options:0 animations:^{
            self.inputBox2.transform = box2Translation;
        } completion:^(BOOL finished) {}];
        [UIView animateKeyframesWithDuration:0.2 delay:0.3 options:0 animations:^{
            self.inputBox3.transform = box3Translation;
        } completion:^(BOOL finished) {}];
        [UIView animateKeyframesWithDuration:0.2 delay:0.4 options:0 animations:^{
            self.inputBox4.transform = box4Translation;
        } completion:^(BOOL finished) {}];
...
- (IBAction)formatPhoneNumber:(id)sender forEvent:(UIEvent *)event {
    if (self.phoneTF.text.length > self.currentPhoneNumber.length) {
        [self.phoneTF setText:[self.numberFormatter inputDigit:[self.phoneTF.text substringFromIndex:self.phoneTF.text.length - 1]]];
    } else {

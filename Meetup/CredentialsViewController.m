...
- (void)viewDidLoad {
    ...
    [self.nameField setValue:[UIColor colorWithWhite:0.75 alpha:1]
                  forKeyPath:@"_placeholderLabel.textColor"];
    [self.phoneField setValue:[UIColor colorWithWhite:0.75 alpha:1]
                   forKeyPath:@"_placeholderLabel.textColor"];
    [self.continueButton.titleLabel
        setFont:[UIFont fontWithName:@"OpenSans" size:22]];
    ...
    dispatch_after(popTime, dispatch_get_main_queue(), ^(void) {
        ...
        CAAnimation* animation = [CAKeyframeAnimation
            animationWithKeyPath:@"position"
                       ...
                       fromPoint:CGPointMake(self.bigLabel.center.x,
                                             self.bigLabel.center.y)
                         toPoint:CGPointMake(self.bigLabel.center.x,
                                             self.bigLabel.center.y + 120)];
        animation.duration = 0.6;
        animation.fillMode = kCAFillModeForwards;
        animation.removedOnCompletion = NO;
        [self.bigLabel.layer addAnimation:animation forKey:@"easing"];

        CAAnimation* animation_smallLabel = [CAKeyframeAnimation
            animationWithKeyPath:@"position"
                        function:ExponentialEaseOut
                       fromPoint:CGPointMake(self.smallLabel.center.x,
                                             self.smallLabel.center.y)
                         toPoint:CGPointMake(self.smallLabel.center.x,
                                             self.smallLabel.center.y + 120)];
        ...
        [self.smallLabel.layer addAnimation:animation_smallLabel
                                     forKey:@"easing"];
    });
}
